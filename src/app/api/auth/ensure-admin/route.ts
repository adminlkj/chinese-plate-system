...
/**
 * Ensure Admin Endpoint — SECURED WITH CRYPTOGRAPHIC TOKEN
 *
 * SECURITY: Same as /api/admin/seed. Creates admin only when none exists.
 * ALWAYS requires ADMIN_SEED_TOKEN regardless of environment.
 * If ADMIN_SEED_TOKEN is not configured, returns 503 Service Unavailable.
 *
 * DEPRECATED: Use /api/admin/seed instead. This endpoint is kept for backward compatibility.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[auth/ensure-admin] Error:', error);
